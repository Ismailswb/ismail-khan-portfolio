    ...
    >
      {previous ? (
        <Link
          ...
          className="group rounded-2xl border border-border bg-surface/70 p-5 transition-all hover:border-border-strong"
        >
          <span className="inline-flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-muted">
            <ArrowLeft className="size-3.5" aria-hidden="true" />
            Previous
          </span>
          <span className="mt-2 block text-base font-semibold text-foreground group-hover:text-accent">
            {previous.title}
          </span>
        </Link>
      ) : (
      ...
      {next ? (
        <Link
          ...
          className="group rounded-2xl border border-border bg-surface/70 p-5 text-right transition-all hover:border-border-strong sm:justify-self-end"
        >
